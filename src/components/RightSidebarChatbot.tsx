...
import { Bot, Send, ChevronRight, ChevronLeft, Wrench, ShieldCheck, RefreshCw, MessageSquare, Sparkles, CornerDownLeft } from 'lucide-react';
...
export const RightSidebarChatbot: React.FC<RightSidebarChatbotProps> = ({ onAddLog }) => {
  ...
  // Strictly scroll ONLY the inner chatbot container down (never the main window/footer)
  useEffect(() => {
  ...
  // Dynamic Contextual Recommended Prompts
  ...
  // Update Dynamic Prompts based on tool execution
  ...
  const handleSendMessage = async (userText: string) => {
    ...
    const userMsg: AgentChatMessage = {
      id: `u-${Date.now()}`,
      senderAgentId: 'user',
      senderAgentName: 'You (Dev / Operator)',
      senderRole: 'User',
    ...
    try {
      // Call backend API /api/v1/agent-chat
      ...
      // Update contextual prompts dynamically
      if (data.toolExecuted) {
        updatePromptsBasedOnTool(data.toolExecuted);
      }

      // Append AI Agent & Tool reply with dynamic response
  ...
  if (!isOpen) {
    return (
      <button
        ...
        title="Open Agent Swarm & Tool Copilot"
      >
        <Bot className="w-6 h-6 animate-pulse" />
        <span className="text-xs font-bold font-mono">Agent Copilot</span>
  ...
  return (
    <aside className="w-full lg:w-96 glass-panel border-l border-slate-800/90 bg-slate-950/95 flex flex-col h-[calc(100vh-65px)] sticky top-[65px] z-40 shadow-2xl transition-all">
      ...
      <div className="p-4 border-b border-slate-800/90 flex items-center justify-between bg-slate-900/60 shrink-0">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-mono">x402 & MCP Tool Engine</p>
          </div>
        </div>
      ...
      {/* Sub-tabs: Swarm Chat vs Tools / MCP Output */}
        ...
        <div ref={messagesContainerRef} className="flex-1 overflow-y-auto p-4 space-y-3 font-sans text-xs min-h-0">
          ...
          {loading && (
            <div className="flex items-center gap-2 text-xs text-indigo-400 font-mono p-2 italic">
              ...
              Agent Swarm evaluating query & executing tools...
            </div>
          )}
        </div>
      )}

      {/* Sub-Tab 2: Tools & MCP Inspection Output */}
      {activeSubTab === 'tools' && (
      ...
      <div className="p-3 border-t border-slate-800/90 bg-slate-900/90 space-y-2 shrink-0">
        ...
        <div className="space-y-1.5">
          <div className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider flex items-center gap-1">
            ...
            <span>Recommended Dashboard Prompts</span>
          </div>
        ...
        <form onSubmit={handleSubmitForm} className="flex items-center gap-2 pt-1">
          <input
            ...
            placeholder="Click prompt above or type question..."
